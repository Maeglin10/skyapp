...
export class AgentsService {
  ...
  run(dto: RunAgentDto) {
    const provider = (dto.provider ?? 'anthropic') as 'anthropic' | 'openai' | 'gemini';
    const model = provider === 'openai' ? 'gpt-4o-mini' : provider === 'gemini' ? 'gemini-2.0-flash' : 'claude-haiku-4-5-20251001';
