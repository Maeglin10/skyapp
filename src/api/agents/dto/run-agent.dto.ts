...
export class RunAgentDto {
  ...
  @ApiPropertyOptional({
    enum: ['anthropic','openai','gemini','skymodel'],
    default: 'anthropic',
    description: "Fournisseur LLM. 'skymodel' = modèle local — requiert make serve-mlx ou make serve-gguf",
  }) @IsEnum(['anthropic','openai','gemini','skymodel']) @IsOptional() @Transform(({ value }) => value || 'anthropic') provider?: string;
